...
| 2026-07-12 | `src/features/FirstTimeHints/FirstTimeHint.tsx` | component | Inline one-time hint label superseded — all first-time hints now use the full-screen spotlight pattern (SpotlightFirstGoal/FirstPush/CheerDiscovery/FirstResponse). No importers remained. | `SpotlightFirstPush` and siblings | deleted | Render-count cap logic stays in the provider (spotlights use it). |
| 2026-07-05 | "Task" vocabulary (client-wide) | rename | New product direction is goal/motivation-first; "Task" naming was from the old TaskSwap app. Full client sweep: `AddTask→AddGoal`, `TaskDetail→GoalDetail`, `Tasks→Goals` features; routes `TaskDetail→GoalDetail`, `AddTask→AddGoal`; types/hooks/api identifiers Task→Goal. `TaskSwapToast→PushMeUpToast`; fixed `NotifcationMainScreen` typo. | Goal vocabulary | replaced | Server contract untouched: `/tasks` URLs, `task-*` notification types, payload fields (`taskId`, `taskType`), and `recentTasks` response field keep old names until the backend renames them. |
