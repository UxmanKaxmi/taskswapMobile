...
import { useGoalPushes } from '@features/Goals/hooks/useGoalPush';
import SectionHeader from '@shared/components/SectionHeader/SectionHeader';
import PushSupportCard from './PushSupportCard';
import GoalProgressUpdateHistory from './GoalProgressUpdateHistory';
...
function buildMockBeats(count: number): GoalBeat[] {
  const now = Date.now();
...
export default function MotivationDetail({
  ...
  const { data: pushData } = useGoalPushes(taskId);
  ...
  return (
    <>
      ...
      {/* )} */}
      {/* <GoalDetailProgress task={task} isOwner={isOwner} hasPushed={hasPushed} /> */}
    </>
  );
}
