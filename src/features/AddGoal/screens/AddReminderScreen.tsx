...
export default function AddReminderScreen({ navigation, route }: Props) {
  ...
  useEffect(() => {
    ...
    if (!draft || draft.type !== GoalTypeEnum.Reminder) return;
  ...
  return (
    <Layout
      ...
      footerContent={
        <AnimatedBottomButton
          title={getButtonText(GoalTypeEnum.Reminder)}
          visible={canSubmit}
          isLoading={isPending}
          onPress={onSubmit}
    ...
    >
      ...
      >
        <GoalBackground icon={typeIcons.reminder} color={typeBackgroundsHard.reminder} />

        <AppHeader title="" />

        <TextElement variant="title" style={styles.subtitle}>
          {getTitle(GoalTypeEnum.Reminder)}
        </TextElement>
        ...
        <Shadow size="high" color={typeBackgrounds.reminder}>
          <View style={styles.inputCard}>
            <GoalDescriptionInput
              value={text}
              onChange={setText}
              placeholder={getGoalPlaceholder(GoalTypeEnum.Reminder)}
              taskType={GoalTypeEnum.Reminder}
            />
          </View>
        </Shadow>
